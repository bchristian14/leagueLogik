...
class Settings(BaseSettings):
    ...
    # API Configuration
    api_v1_str: str = Field(default="/api/v1", description="API version 1 path prefix")
    ...
    environment: str = Field(
        default="production", description="Environment name (development, production)"
    )
    ...
    admin_email: str = Field(
        default="admin@leaguelogik.com", description="Default admin user email address"
    )
    admin_password: str = Field(
        default="admin123!", description="Default admin user password"
    )
    admin_first_name: str = Field(default="League", description="Admin user first name")
    admin_last_name: str = Field(
        default="Administrator", description="Admin user last name"
    )
