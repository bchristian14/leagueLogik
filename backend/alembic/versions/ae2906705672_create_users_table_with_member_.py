...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "member_status",
            sa.Enum("ACTIVE", "INACTIVE", name="memberstatus"),
            nullable=False,
        ),
        sa.Column(
            "member_type",
            sa.Enum("CANDIDATE", "FULL", "LIFETIME", name="membertype"),
            nullable=False,
        ),
        sa.Column(
            "admin_roles",
        ...
        sa.CheckConstraint(
            "member_balance >= -500.00", name="check_member_balance_limit"
        ),
    ...
    op.create_index(
        "idx_users_name_search", "users", ["last_name", "first_name"], unique=False
    )
    op.create_index("idx_users_signup_date", "users", ["signup_date"], unique=False)
    op.create_index(
        "idx_users_status_type", "users", ["member_status", "member_type"], unique=False
    )
    ...
    op.create_index(
        op.f("ix_users_member_status"), "users", ["member_status"], unique=False
    )
    op.create_index(
        op.f("ix_users_member_type"), "users", ["member_type"], unique=False
    )
